...
const Menubar = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, MenubarRootProps<T>>) => {
  const [local, others] = splitProps(props as MenubarRootProps, ['class']);
  return (
    <MenubarPrimitive.Root
      class={cn('flex h-10 items-center space-x-1 rounded-md border bg-background p-1', local.class)}
      {...others}
    />
  );
};
...
type MenubarTriggerProps<T extends ValidComponent = 'button'> = MenubarPrimitive.MenubarTriggerProps<T> & {
  class?: string | undefined;
};

const MenubarTrigger = <T extends ValidComponent = 'button'>(props: PolymorphicProps<T, MenubarTriggerProps<T>>) => {
...
type MenubarContentProps<T extends ValidComponent = 'div'> = MenubarPrimitive.MenubarContentProps<T> & {
  class?: string | undefined;
};

const MenubarContent = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, MenubarContentProps<T>>) => {
...
const MenubarSubTrigger = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, MenubarSubTriggerProps<T>>) => {
  const [local, others] = splitProps(props as MenubarSubTriggerProps, ['class', 'children', 'inset']);
  return (
...
const MenubarSubContent = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, MenubarSubContentProps<T>>) => {
...
const MenubarItem = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, MenubarItemProps<T>>) => {
...
const MenubarRadioItem = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, MenubarRadioItemProps<T>>) => {
...
const MenubarItemLabel = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, MenubarItemLabelProps<T>>) => {
  const [local, others] = splitProps(props as MenubarItemLabelProps, ['class', 'inset']);
  return (
    <MenubarPrimitive.ItemLabel
      class={cn('px-2 py-1.5 font-semibold text-sm', local.inset && 'pl-8', local.class)}
      {...others}
    />
  );
};
...
type MenubarSeparatorProps<T extends ValidComponent = 'hr'> = MenubarPrimitive.MenubarSeparatorProps<T> & {
  class?: string | undefined;
};

const MenubarSeparator = <T extends ValidComponent = 'hr'>(props: PolymorphicProps<T, MenubarSeparatorProps<T>>) => {
...
const MenubarShortcut: Component<ComponentProps<'span'>> = (props) => {
  ...
  return <span class={cn('ml-auto text-muted-foreground text-xs tracking-widest', local.class)} {...others} />;
};
