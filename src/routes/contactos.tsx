...
import { getSeoSettings, getSocialNetworksAndContacts } from '~/cms/services/singletons/settings';
...
export const route: RouteDefinition = {
  preload: async () => {
    return Promise.all([getContacts(), getSocialNetworksAndContacts(), getSeoSettings()]);
  },
};

function ContactsRouter() {
  const data = createAsync(() => getContacts(), { deferStream: true });
  const listOfContacts = createAsync(() => getSocialNetworksAndContacts(), { deferStream: true });
